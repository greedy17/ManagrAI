...
def check_failed_operations(sender, instance=None, created=False, **kwargs):
    ...
    if created:
        ...
        if (
            ...
            or instance.attempts <= 5
        ):
            return

        if queue == sf_consts.SALESFORCE_RESOURCE_SYNC_QUEUE and instance.attempts >= 5:
            sync_id = json.loads(instance.task_params)[0][1]
            operation = SFSyncOperation.objects.filter(id=sync_id).first()
        elif queue == sf_consts.SALESFORCE_MEETING_REVIEW_WORKFLOW_QUEUE and instance.attempts >= 5:
            sync_id = json.loads(instance.task_params)[0][1]
            operation = MeetingWorkflow.objects.filter(id=sync_id).first()
