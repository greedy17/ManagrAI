...
def OPEN_AI_NEWS_CLIPS_SUMMARY(date, clips, search, instructions=False, for_client=False):
    ...
    Cite your sources by enclosing the index of the search result in a set square brackets and parenthesis at the end of the corresponding sentence, without a space between the last word and the citation. For example: 'Paris is the capital of France[1](1).' This is being used in a Javascript app so indexes must always start with 0. Only use this format to cite search results.
