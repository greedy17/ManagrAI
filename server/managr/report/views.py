...
class StoryReportViewSet(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not opportunity:
            raise ValidationError({"opportunity": "required field"})
        try:
        ...
        report = StoryReport.objects.create(
            opportunity=opportunity, generated_by=generated_by
        )
        emit_report_event(str(report.id), report_const.STORY_REPORT)
        return Response(
            data=self.serializer_class(report).data, status=status.HTTP_200_OK
        )
