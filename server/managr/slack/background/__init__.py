...
def emit_send_paginated_inline_alerts(payload, context):
    _prcocess_send_paginated_inline_alerts(payload, context)
...
@background(schedule=0)
@log_all_exceptions
def _prcocess_send_paginated_inline_alerts(payload, context):
    ...
    for alert_instance in instances.get("results", []):
        ...
        if value in ["dealstage", "StageName"]:
            field_block["accessory"][
                "action_id"
            ] = f"{slack_const.COMMAND_FORMS__STAGE_SELECTED}?u={str(user.id)}&f={str(form.id)}"
        blocks.append(field_block)
    if len(blocks):
        blocks = [
            *blocks,
            *custom_inline_paginator_block(instances, invocation, channel, config_id),
        ]
    slack_requests.generic_request(
        payload["response_url"],
        {"replace_original": True, "blocks": blocks},
